...
Class(Зависимость* dep) : dep(dep) {}

// Абстрактный базовый класс для всех окон-блоков
...
public:
    ...
    void showExpressEditWindow() override {
        ...
        // Создаем окно редактора с repository
...
void Operation_window::addBlock() {
    ...
    // ... добавление блока в UI
}
```
**Когда:** По умолчанию. Для тестируемости, явных зависимостей.
 еще ХОРОШАЯ практика для продакшена  **Dependency Injection (внедрение зависимостей)** через callback/функциональные объекты. Вот почему это хорошо:
## 2. **Service Locator**
```cpp
...
// Operation_window соединяет сигналы:
void Operation_window::addBlock() {
...
void Operation_window::onRequestEquipmentData(int id) {
    ...
    // Отправляем данные обратно (нужен механизм идентификации запроса)
}
```
**Когда:** Развязанные UI-компоненты, асинхронные запросы.
...
```
**Когда:** Множество реализаций, плагины, runtime-выбор.
...
```
**Когда:** Простые приложения, тесная связь допустима.
___
## **Signal-Slot с контекстом (Qt-стиль)**
```cpp
...
- Qt приложения с динамическим UI
- Когда зависимости становятся доступны позже

## **Поиск через родительскую цепочку**
```cpp
...
```
**Когда использовать:**
- Иерархические оконные приложения
- Когда нельзя менять конструкторы
...
## 🎯 Быстрый выбор:
- **Новый код** → DI (1)
- **Рефакторинг** → Service Locator (2) → постепенно к DI
- **Qt виджеты** → Сигналы (3)
- **Плагины/типы** → Менеджер (4)
- **Быстро/прототип** → Ссылка на родителя (5)

**Золотое правило:** Начинай с DI, отступай только при веских причинах.